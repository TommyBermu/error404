...
def _rewrite_chain(
    ...
    """Persist previous/next (and optional order) pointers to match ``ordered_nodes``."""
...
def get_ordered_modules(course: Course) -> List[Module]:
    """Return course modules ordered using their linked pointers."""
    modules = list(
...
def get_ordered_contents(module: Module) -> List[Content]:
    """Return module contents ordered using their linked pointers."""
    contents = list(
